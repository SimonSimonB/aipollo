...
class StaffDetector:
    ...
    def _detect(self, image):
        ...
        for connected_component in line_segments[:10]:
            ...
            best_line = max(lines_and_scores, key=lambda line_and_line_pixel_sum: line_and_line_pixel_sum[1])
        ...
        lines_and_scores = sorted(lines_and_scores, key=lambda line: line[1], reverse=True)[:round(0.1 * image.shape[0])]
        ...
        staff_line_distances = sorted(lines_and_scores[i + 1][0][0].y - lines_and_scores[i][0][0].y for i in range(len(lines_and_scores) - 1))
        ...
        for i in range(1, len(lines_and_scores)):
            if lines_and_scores[i][0][0].y - lines_and_scores[i - 1][0][0].y > max_staff_line_distance:
                current_staff = []
        ...
        staffs = [
            [ScoreElement(ScoreElementType.staff_line, staff_line) for staff_line in staff] 
            for staff in staffs
        ]
