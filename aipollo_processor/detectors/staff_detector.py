...
class StaffDetector:
    ...
    def detect(self, image):
        ...
        staff_height = sum(staff[4][0][0].y - staff[0][0][0].y for staff in staffs) / len(staffs)
        ...
        return staffs, staff_height * (original_height / resize_to_height)
    ...
    def _detect(self, image):
        ...
        mask_uint *= 255
        fast_line_detector = cv2.ximgproc.createFastLineDetector(10, 1.414, 50, 50, 3, True)
        lines = fast_line_detector.detect(mask_uint)
        line_segments = [geometry_utils.get_line_segment
            (geometry_utils.Point(y=line[0][1], x=line[0][0]), geometry_utils.Point(y=line[0][3], x=line[0][2]))
            for line in lines]

        # Throw away lines that span less than 25% of the image
        #line_segments = [line_segment for line_segment in line_segments if len(line_segment) > 0.25 * image.shape[1]]
        ...
        line_segments.sort(key=lambda line_segment: len(line_segment), reverse=True)
        lines = []
        ...
        for shift in range(-best_line[0].y, mask.shape[0] - best_line[0].y):
            line_candidate = geometry_utils.get_line(geometry_utils.Point(best_line[0].y + shift, best_line[0].x), geometry_utils.Point(best_line[-1].y + shift, best_line[-1].x), mask.shape[0], mask.shape[1])
            line_candidate_pixel_sum = sum(mask[p.y][p.x] for p in line_candidate)
        ...
        line_candidates = sorted(line_candidates, key=lambda line: line[0][0].y) 
        i = 0
        while i < len(line_candidates) - 1:
            if line_candidates[i + 1][0][0].y - line_candidates[i][0][0].y < 3:
        ...
        staff_line_distances = sorted(line_candidates[i + 1][0][0].y - line_candidates[i][0][0].y for i in range(len(line_candidates) - 1))
        ...
        for i in range(1, len(line_candidates)):
            if line_candidates[i][0][0].y - line_candidates[i - 1][0][0].y > max_staff_line_distance:
                current_component = []
